...
echo "composfile is $DR_MINIO_COMPOSE_FILE, press anykey to continue"
if [[ -n "${DR_MINIO_COMPOSE_FILE}" ]]; then
  ## using id -u, id -g  will cause problem on macOS
    ...
    ## add comment
    echo " root: $DR_LOCAL_SECRET_ACCESS_KEY acckey:  $DR_LOCAL_ACCESS_KEY_ID  , DR_DIR: $DR_DIR "
    if [[ "${DR_DOCKER_STYLE}" == "swarm" ]];
    then
        echo "doing with stack deploy , press anykey to continue $"
        read
        docker stack deploy $DR_MINIO_COMPOSE_FILE s3
    else
        echo "doing with compose, press anykey to continue"
        read
